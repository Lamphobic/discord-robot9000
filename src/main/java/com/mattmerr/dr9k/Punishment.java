...
public class Punishment{
    ...
    String getHumanTimeRemaining(){
        ...
        if(years > 0) ret += String.format("%d years, ", years);
        if(months > 0) ret += String.format("%d months, ", months);
        if(weeks > 0) ret += String.format("%d weeks, ", weeks);
        if(days > 0) ret += String.format("%d days, ", days);
        if(hours > 0) ret += String.format("%d hours, ", hours);
        if(minutes > 0) ret += String.format("%d minutes, ", minutes);
        if(seconds > 0) ret += String.format("%d seconds", seconds);
        return ret;
    }
    ...
    public boolean equals(Object o) {
        ...
        return Objects.equals(severityLevel, e.severityLevel) && Objects.equals(punishmentStart, e.punishmentStart);
    }
    ...
    public String toString() {
        return String.format("Severtiy %d: Start %s", severityLevel, punishmentStart.toString());
    }
}